        ...
        button.setOnClickListener(new View.OnClickListener(){
            public void onClick(View view){
                ...
                if (cursor.getCount()==0)
                {
                    ...
                    contentkey = "&content=";
                    try {
                        connectionparamters = emailkey + URLEncoder.encode(getemail, "UTF-8") + sponserkey
                                + URLEncoder.encode(cont.getText().toString(), "UTF-8") + contentkey
                                + URLEncoder.encode(spon.getText().toString(), "UTF-8");

                        paramtersbyt = connectionparamters.getBytes("UTF-8");
                        Toast.makeText(getActivity(), getemail+"\n"
                                        +cont.getText().toString()+"\n"
                                        +spon.getText().toString()
